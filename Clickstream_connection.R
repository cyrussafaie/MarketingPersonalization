...
### 1) visitor id (integer vector of unit length),
### 2) visitor features (integer vector of length 93);
## and to return a vector of predicted probabilities (one probability for each of 5 categories)
Connect <- function(host, port, login, password, stream_name, event_handler, catch_handler_errors=TRUE) {
    ...
    repeat {
        raw_msg <- ReceiveRawMsg(con);
        event_msg <- Clickstream.Event$read(raw_msg);
        # check errors
        ...
        # process new visitor from server
        if (event_msg$has('features')) {
            predicted_probabilities <- NULL
            tryCatch({
                predicted_probabilities <- event_handler(event_msg$visitor_id, event_msg$features);
                # assert prob length and value
                stopifnot(is.numeric(predicted_probabilities), length(predicted_probabilities) == 5, 
                          predicted_probabilities >= 0, predicted_probabilities <= 1, 
                          sum(predicted_probabilities) > 0.999, sum(predicted_probabilities) < 1.001);
            }, error = function(e) {
                if (catch_handler_errors) {
                    problem <- paste("Error inside handler: ", e, 'Forcing uniform distribution!', sep='');
                    message('!!!***   WARNING   ***!!!\n', 
                            problem,
                            '\n!!!*******************!!!');
                    current_problem_n <<- current_problem_n + 1;
                    result$problems[current_problem_n,] <<- list(Sys.time(), problem);
                    predicted_probabilities <<- rep(1,5) / 5;  # uniform distribution
                }
                else {
                    stop(e);
                }
            ...
            message("Sending predicted probabilities for visitor_id=", event_msg$visitor_id);
            signal_msg <- Clickstream.Signal$new(visitor_id = event_msg$visitor_id, 
...
message("Clickstream_connection.R sourced!");
